...
{	
	...
	{	
		...
		cost = scr_ability_excecute_script( 1, menu_options[| i * 4 + 1 ], menu_id, player_core );
		...
		if( point_distance( len_x,len_y,m_x,m_y ) < 26 ) // mouse is over this option
		{
			...
			draw_text_colour( 160, 30,  "Time: " + string( scr_ability_excecute_script( 2, menu_options[| i * 4 + 1 ], menu_id, player_core ) ), c_red,  c_red,  c_red,  c_red,  1 );
		...
		draw_sprite_ext( menu_options[| 4 * i ], 0, len_x, len_y, 1, 1, 0, col, 1 );
	}
